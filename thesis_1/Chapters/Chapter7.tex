...
%	Nous avons d'abord analyser la perception des comportements de pouvoir lors de la négociation. En effet, avant d'analyser les hypothèses, nous voudrions étudier la perception des comportements de complémentarité et la similarité durant la négociation. 
	...
	\subsection{Comportement de l'agent neutre}
	
		Nous avons conduit les mêmes études statistiques pour analyser la perception des comportements de l'agent neutre. L'analyse descriptive présentée dans la figure .. montre que les participants ont perçu que l'agent adoptait une stratégie complémentaire pour les comportements \textbf{D1}, \textbf{D2} et \textbf{D3}. Cependant, aucune différence significative n'a été perçu pour le comportement \textbf{D4}.
...
Quatre comportements de pouvoir ont été pris en compte pour mesurer la stratégie de négociation, les participants ont été en mesure de distinguer une différence significative sur trois comportements entre leurs stratégie et celle de l'agent complémentaire.

En effet, les participants ont perçu une différence entre leurs niveau d'exigence et de concessions, ainsi que la prise en compte des préférences de l'autre. Cependant, aucune différence n'a été perçu concernant les comportements de leadership durant la négociation. 

Ces mêmes résultats ont été trouvé en analysant l'agent neutre. 
En effet, les comportements de l'agent neutre ont été perçu comme significativement complémentaires à ceux des utilisateurs pour les comportements \textbf{D1}, \textbf{D2} et \textbf{D3}. 

Cependant comme l'agent complémentaire, aucune différence n'a été perçue dans les comportements relatifs à \textbf{D4} entre l'agent neutre et le participant. 

Concernant l'agent complémentaire, l'absence de per
était due à l'adaptation de la stratégie de l'agent à chaque tour qui aurait pu masquer les comportements de leadership durant la négociation.
En outre, l'aspect collaboratif de la négociation pourrait avoir atténué la perception du leadership dans la négociation. 
...
De plus, la perception des comportements de l'agent neutre comme complémentaires à ceux des participants soutient que la relation de dominance est complémentaire \cite{burgoonnonverbal}. En effet, indépendamment des deux autres agents où nous avions manipulé l'adaptation de l'agent, dans la condition neutre l'agent ne s'adapte pas aux comportements du participant. Néanmoins, les résultats démontrent que le participant s'est adapté aux comportements exhibé par l'agent et a ainsi établie une relation interpersonnelle de dominance complémentaire.

\subsection{Gain commun}
	Notre seconde hypothèse \textit{Les négociateurs atteignent un gain commun plus important quand les négociateurs établissent une relation de dominance complémentaire.} est fortement validé. 
	
	Expliquer que l'analyse objective a donné de bons résulats, même 
	
	Les participants ont été satisfaits du choix final et le trouvaient équitable pour tout les agents. 
	En effet, en analysant le valeur de satisfiabilité du restaurant trouvé pour les préférences des participants. Les valeurs étaient autours de 0.7 (min =0.69, max = 0.72). 
	Cependant, en analysant la valeur de sat pour les préférences de l'agent, seul l'agent complémentaire a pu converger vers un restaurant qui respecte ses préférences. 
	
	l'analyse: Négociation durait longtemps avec l'agent similaire ce qui faisait augmenter les comportements de concession chez l'agent, et potentiellement finir par accepter un restaurant qui n'est pas satisfiable (m = 0.33). 
	Concernant l'agent neutre qui n'est pas très dominant, il est normal que dans la majorité des cas, la courbe de concession allait finir par décroitre menant l'agent à faire des concessions. 
	
	Ces résultats expliquent pourquoi dans l'analyse objective, le gain commun était plus important durant les négociations avec l'agent complémentaire. 
\subsection{Tours de paroles}
	Hypothèse validée
	converger moins rapidement
