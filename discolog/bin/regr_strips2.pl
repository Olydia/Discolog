...
strips_solve(G,N,P) :-
   writeln(['strips_solve(',G,',',N,',',P,').']),
   strips_solve(G,[G],N,P).

strips_solve(G,_,_,init) :-
   writeln(['strips_solve(',G,',_,_,',init,').']),
   strips_solved(G).

strips_solve(G,AS,NAs,do(A,Pl)) :-
   writeln(['strips_solve(',G,',',AS,',',NAs,',do(',A,',',Pl,')).']),
   NAs > 0,
...
strips_useful([S|R],A) :-
   writeln(['strips_useful(',[S|R],',',A,').']),
   strips_holds(S,init),
   strips_useful(R,A).
strips_useful([S|_],A) :-
   writeln(['strips_useful([',S,'|_]',',A,).']),
   strips_achieves(A,S).
strips_useful([S|R],A) :-
   writeln(['strips_useful(',[S|R],',',A,').']),
   \+ strips_holds(S,init),
...
strips_wp([],A,G1) :-
   writeln(['strips_wp([],',A,',',G1,').']),
   strips_preconditions(A,G),
   writeln(['calling strips_filter_derived']),
   strips_filter_derived(G,[],G1).
strips_wp([S|R],A,G1) :-
   writeln(['strips_wp(',[S|R],',',A,',',G1,').']),
   strips_wp(R,A,G0),
...
strips_regress(S,A,G,G) :-
   writeln(['strips_regress(',S,',',A,',',G,',',G,').']),
   strips_achieves(A,S).
strips_regress(S,A,G,G1) :-
   writeln(['strips_regress(',S,',',A,',',G,',',G1,').']),
   strips_primitive(S),
   \+ strips_achieves(A,S),
   \+ strips_deletes(A,S),
...
strips_filter_derived([G|R],L,[G|L1]) :-
	writeln(['strips_filter_derived(',[G|R],',',L,',',[G|L1],').']),
   strips_primitive(G),
