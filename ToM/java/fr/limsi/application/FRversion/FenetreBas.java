...
public class FenetreBas extends AnchorPane{
	...
	public FenetreBas(String agentName){
	 ...
	 * @param situation: restaurant or movie
	 */
	public void clearLastButtons(ArrayList<HBox> c1HBoxs,ArrayList<HBox> c2HBoxs,
			ArrayList<HBox> c3HBoxs,ArrayList<HBox> c4HBoxs,ObservableList list,String situation){
	...
	public void addElements(String username, String situation,Stage chatStage ,
		...
		stopButton.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent prosposeEvent) {
				...
				ShowPreferencesDialog userPref = new ShowPreferencesDialog(PrefAcceuil.preferencesUser);
			...
			private void nextStep(int nbAgents) {
				...
				if (result.isPresent()){
					if(result.get().equals(passWord)){
						...
						switch(nbAgents){
						case(0):{
							FenetreHaut chat2 = new FenetreHaut("Arthur",username, ADAPT.MIMIC);
							chat2.situation="restaurant";

							String arthur = "--------------------------------------------- \n \n"+
									"Preferences of agent Arthur \n \n" +
									PrefAcceuil.negotiators.get(1).printPreferences();

							writer.write(arthur, fichier2);
							chatStage.hide();

							chat2.setPrefModel(PrefAcceuil.getNegotiators().get(1));
