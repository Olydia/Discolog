...
%	
	...
	%\par One assumption underlying symbolic planning is that planning problems are deterministic \cite{cimatti1998strong}. This means that the environment is fully defined to allow the prediction of all the possible changes in this environment: the action execution always lead to the expected state. However, planners deal with highly dynamic environments which are unpredictables and can not be completely represented. Therefore, the executed plan faces breakdowns due to the lack of knowledge.
