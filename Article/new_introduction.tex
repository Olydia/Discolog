...
\begin{itemize}
\item  Basic structure of linear planning  STRIPS 

action structure + planning solution  \cite{fikes1972strips}
\item HTN architecture: 
hiearchical structure in AND/OR tree. Compound task . Planning : task decomposition until primitive tasks are reached. \cite{erol1996hierarchical}
\subsection{Related works and previous approachs}

	\subsubsection{Reactive planning }
 \item Reactive planning becomes very popular in AI such in controlling mobile agents \cite{beaudry2005reactive}, simulating human behaviour \cite{bryson2001modularity} or Gaming with the name f behaviour trees. Behaviours trees have the same hiearchical structure of HTNs and do Real time decision which can be seen as reactive planning.
 ...
 \subsubsection{Plan repair}
 	\item plan repair by extending the generated plan with graph containing the causal links between the HTN's tasks
 	 \cite{ayan2007hotride}  \cite{warfield2007adaptation} \cite{van2005plan} \cite{boella2002replanning}
 	 \item plan repair using heurestic \cite{hayashi2006dynagent}
  \end{itemize}
