...
\section{motivation}
		motivation : reactive HTNs plans only with reactive knowledge that not represent completely the environment.
		For consequence, the HTN plans only for the next step and an action execution can leads the HTN to a state where no action in the hierarchy can be applicable. For example ( For the purpose of clarity we use a simple toy example).
	...
	In this section we present our proposition based on exetending the reactive HTN with symbolic planner to recover from breakdown.
	reactive HTN  breakdown
	Procedural knowledge has the form of boolean procedure. this later can thought as predicate. We propose to the HTN designer to extend theses procedure formalism to a symbolic one. Of  course this is cannot be done on the entire procedre domain knowledge
	Thus, if a breakdown occurs (The agent has nothing to execute and the goal is not reached yet). The proposed system will first detect all the tasks in the HTN affected by the breakdown, and exactelly which of it pre/post conditions are no longer supported in the current state. Once all the candidates are detected, the linear planner is called for each candidate and the most promising plan is keeped and passed to the reactive HTN to be executed. 
