...
%	\end{table*}
	...
	We evaluate the time execution of the algorithm in order to study how the model of theory of mind evolves.For each dialogue, we computed the time execution of the negotiation. Results are presented in table \ref{tab:time}, where wee see the evolution ofthe time execution. Unfortunately, we can observe that the evolution of our algorithm is exponential. However, it managed to give accurate predictions in realistic time
...
%	
	\section{Discussion}
		We presented model of theory of mind that aims to predict the value of power expressed by the agent's partner.
		
		
	\section{Conclusion}
	
		Rappeler la contribution
		collaboratif negotiation
		model de la theorie de l'esprit
		evaluation avec bon resultats
		meme avec des conaissances partielles on pouvait 
		limites evaluer dans le context d'interaction avec homme machine
