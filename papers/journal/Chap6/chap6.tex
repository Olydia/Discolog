...
\usepackage{subfig} 
\usepackage{graphicx}
\usepackage{frame, caption,adjustbox}
\usepackage{amsmath}
\usepackage{eulervm}
\usepackage{fontenc}
\usepackage{mathrsfs}
...
First, he explained that the purpose of the interaction was to negotiate with each agent to find a restaurant where to have dinner. He gave them instructions to project themselves into a real situation in addition to emphasizing the "collaborative" aspect of negotiation as presented in the figure \ref{fig:instruction}.
	...
	\begin{figure}[h]
		\fbox{\begin{minipage}{.95\textwidth}
				{\ttfamily
					\textbf{Instruction:}  Imagine a situation where you are having dinner with a friend or colleague for the first time, you do not know their tastes and they do not know yours. The goal is to negotiate according to your respective preferences to choose a restaurant that is suitable for both of you.}.
...
\begin{figure}[!tb]
	...
	\subfloat[Mean and standard deviation for the behavior of dominance perceived]{
		...
		\begin{tabular}{l c c c c c c}
			...
			\multirow{2}{*}{\textbf{Neutral}}& Mean & 1,99 & 3,81 & 2,28 & 3,02 \\
			& SD & 0,94 & 0,94 & 1,01 & 1,12 \\
			
			\hline
			\multirow{2}{*}{\textbf{Part}}& Mean & 2,98 & 2,88 & 3,60 & 3,17 \\
			
			& SD & 1,08 & 1,12 & 1,01 & 1,10 \\
			\hline \hline
...
These results suggest that participants have adopted a different negotiation strategy based on the relationship established with the agent.
		...
		\subfloat[Evaluation of participants' perceived dominance behaviors by agents. \textit{the populations grouped with $(*)$ are significantly different }]{
			\includegraphics[clip=false]{figs/pow.png}
		}
		
		\subfloat[Agent's perception of the dominance behaviors expressed by participants]{
			\begin{tabular}{ l c c c c c }
				\hline
				\hline
				\textbf{ }& \textbf{Comp.} & &  \textbf{Similar} & & \textbf{Neutral} \\ 
...
\vspace{-0.75 em}
	...
	\begin{table} [H]
		...
		\begin{tabular}{ |p{2cm}| p{2cm} |p{2cm}| p{2cm}| p{2cm}|}
			\cline{3-5}
			\multicolumn{2}{c|}{ } & \textbf{Comp. $>$ Similaire} & \textbf{Comp. $>$ Neutral} & \textbf{Neutral $>$ Neutral} \\ 
			\hline
	...
	\begin{table}[H]
		\begin{adjustbox}{angle=90}
			\begin{tabular}{ l c p{3 cm} p{3 cm} p{3 cm} }
				...
				\multirow{3}{*} {Appreciation}  & Z-Wilcoxon& -3,17 & -0,32 & -3,29	\\ 	
