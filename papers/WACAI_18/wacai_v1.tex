...
\begin{document}
	...
	\maketitle
	\begin{abstract}
		Cet article présente un agent conversationnel capable de déployer différentes stratégies de négociation en fonction de son pouvoir social. Le modèle computationnel sous-jacent est basé sur trois principes négociation extrait de la littérature en psychologie sociale. Le comportement social de l'agent est rendu visible à travers sa stratégie de dialogue. Nous présentons une expérience qui étudie l'effet du pouvoir sur ns les stratégies éxprimées par l'agent au cours d'une négociation collaborative humain-agent. Nos résultats montrent que les participants perçoivent correctement les différences dans les stratégies d'agent en fonction de la relation de pouvoir qu'il cherche à d'exprimer.
	\end{abstract}

\keywords{Négociation collaborative, Relation de pouvoir, Stratégie de négociation}
