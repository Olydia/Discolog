...
%	\end{itemize}
	...
	%$$score_s(x) = \frac{1}{\sum_{i[1,card(x)]} i} \sum_{i[1,card(x)]} rank(c_i)\times \frac{1}{card(D_{c_i})-1} \frac{score(v_i)+card(D_{c_i})-1}{2} $$
	
	or simply:
	
	$$score_s(u(x)) = normalize \left( \sum_{i[1,card(x)]} rank(c_i)\times normalize ( score(v_i) ) \right) $$
	 
	% such that normalize returns a value $\in \[0, 1\]$.
	
	
	%such that  normalize returns a value $\in \[0, 1\]$.  Similarly, we compute:
