	...
	\begin{document}
		...
		\hl{ Furthermore}, \textit{Nazari et al}\cite{nazari2015opponent} \hl{showed that communicating preferences in a negotiation with a human improve the negotiation outcomes. We consider, in our paper, a negotiation of preferences where negotiators can share their preferences freely. Whereas,} \cite{nazari2015opponent} \hl{considered situations where negotiators used deception to gain an unfair advantage and might hide their intentions.}
	...
	\end{itemize}
		...
		\subsection{Results and discussion}
			\vspace{-.5em} 
			Table  \ref{res} summarizes the results of our study, which strongly support all of our four hypotheses. \hl{ Cette phrase est tres mal tourne mais je nai pas su reformuler.  We first computed the correlation for each pair of questions for a given hypothesis (the average correlation is at .5 for all pairs of questions in all dialogues)}. This allow us to use the data to compare the speaker's behaviors on each dialogue.
			 Since our data are not normally distributed, we used a non-parametric \emph{Wilcoxon signed-rank test} for paired data. 
			 
			 
			 The higher-power speaker was highly perceived as more self-centered (\textbf{H1}), with a large effect size. For dialogue1, the statistical test indicates that the higher-power agent was significantly perceived as more individualist \emph{Z=-5.28, p$<$0.001}. 
			In addition, a significant difference in the level of concessions expressed in all the dialogues was revealed (\textbf{H2}). Indeed, the high-power agent was perceived as making less concessions. The effect size showed a medium effect for the following dialogues: (Dialogue2, Dialogue3, and Dialogue4), and a large effect for Dialogue1 with \emph{Z=-5.34, p$<$0.001}.
			...
			The (\textbf{H4}) hypothesis was confirmed. the Wilcoxon ranked test revealed that the high-power agent was perceived as significantly more leading the dialogue than the low-power agent, with a large effect size for the dialogues (Dialogue1, Dialogue2 and Dialogue 4) and a medium effect size for Dialogue3 as depicted in Table \ref{res}.
