pValue <- function(csvfile){
  ...
  p3 <- wilcox.test(H3_A, H3_B, mu=0, alt ="two.sided", paired =T, conf.int= T, conf.level = 0.8, exact=F, correct = F)
  print(p3)
  
  p4 <-  wilcox.test(H4_A, H4_B, mu=0, alt ="two.sided", paired =T, conf.int= T, conf.level = 0.8, exact=F, correct = F)
  print(p4)
}
